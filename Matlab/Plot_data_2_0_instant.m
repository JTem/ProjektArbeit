...
ax1 = axes('position',[0.0 0.45 0.45 0.55]);
...
ax2 = axes('position',[0.45 0.73 0.52 0.25]);
...
ax3 = axes('position', [0.45 0.4 0.52 0.25]);
title('Curvature'), xlabel('t in %'), ylabel('curvature');hold on;
set(ax3, 'XLim', [0 100]);

% Graph 4
ax4 = axes('position', [0.45 0.07 0.52 0.25]);
title('Angle between optimal vs ordinary Tool-Orientation'), xlabel('t in %'), ylabel('angle in °'); hold on;
set(ax4, 'XLim', [0 100]);

% Graph 5 (noch nicht belegt LOL)
ax5 = axes('position', [0.05 0.07 0.3 0.3]);
title('Freier Graph WIP'), xlabel('t in %'), ylabel('angle in °'); hold on;
set(ax4, 'XLim', [0 100]);
...
for j = 1:length(x)
    ...
    % abspeichern der Flansch-Transformationsmatrix
...
plot3(ax1, x, y, z, 'color', 'b', 'Marker', '.');
plot(ax2, x_plot, aPhi, 'color', 'r'); hold on;
plot(ax2, x_plot, aTheta, 'color', 'g');
plot(ax2, x_plot, aPsi, 'color', 'b');
legend(ax2, 'phi', 'theta', 'psi');

if surf_curv
    yyaxis(ax3, 'right')
    plot(ax3, x_plot, mCurvature, 'color', 'r'); hold on;
    plot(ax3, x_plot, gCurvature, 'color', 'g');
yyaxis(ax3, 'left');
end
plot(ax3, x_plot, curvature_Path, 'color', 'k');hold on;
plot(ax3, x_plot, curvature_Proj, 'color', 'm');
if surf_curv
legend(ax3,'Path-Curv','Projekted-Curv', 'Mean', 'Gaussian');
else
    legend(ax3,'Path-Curv','Projekted-Curv');
end
plot(ax4, x_plot, dPhi, 'b');
ref = refline(ax4,[0 45]);
ref.Color = 'r';
