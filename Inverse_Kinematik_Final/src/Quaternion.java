...
public class Quaternion {
	...
	private void Rot2Quat2(Jama.Matrix R) {
		...
		n = 0.25*Math.sqrt(Math.pow((r11+r22+r33+1), 2)+Math.pow((r32-r23),2)+Math.pow((r13-r31),2)+Math.pow((r21-r12), 2));
		ex = 0.25*Math.signum(r32-r23)*Math.sqrt(Math.pow((r32-r23), 2)+Math.pow((r11-r22-r33+1),2)+Math.pow((r21+r12),2)+Math.pow((r31+r13), 2));
		ey = 0.25*Math.signum(r13-r31)*Math.sqrt(Math.pow((r13-r31), 2)+Math.pow((r21+r12),2)+Math.pow((r22-r11-r33+1),2)+Math.pow((r32+r23), 2));
		ez = 0.25*Math.signum(r21-r12)*Math.sqrt(Math.pow((r21-r12), 2)+Math.pow((r31+r13),2)+Math.pow((r32+r23),2)+Math.pow((r33-r11-r22+1), 2));
		if (Double.isNaN(n))
			n = 0;
		if (Double.isNaN(ex))
			ex = 0;
